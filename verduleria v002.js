        ...
        fetch('./ItemsInFolder.json')
          ...
          .then(data => {
            ...
            data.forEach(objetoJSON => {
                Items.push(new Item(0,objetoJSON.nombre, objetoJSON.precio, objetoJSON.cantidad));
          })
          resolve(Items);
        })
        })
...
botonAgregar.forEach((element) => {
  element.addEventListener("click", () => {
    if (traerPantallaContador(element).innerHTML != 0) {
      ...
      AvisoTemporal(nombreTitulo.innerHTML,cantidad.innerHTML);
      destruirHTMLCarrito();
...
// EL TOASTIFY
function AvisoTemporal(nombre,cantidad) {
  Toastify({
    text: `Agregaste ${cantidad} de ${nombre} al carrito`,
    duration: 3000
    }).showToast();
  }
    ...
    lista.insertAdjacentHTML(
      ...
      `
        <div class="card-group cuadradoGrande">
            <div class="card">
            <img src="https://imsea.herokuapp.com/api/1?q=${element.nombre}" class="card-img-top" alt="...">
    ...
    lugar.insertAdjacentHTML(
      ...
      `
          <div class="esto1">
              ...
              <p class="card-text"> Subtotal:$${element.pxq()}</p>
